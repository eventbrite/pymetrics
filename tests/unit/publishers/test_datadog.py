...
class TestDogStatsdPublisher(object):
    ...
    def test_no_metric_values_does_nothing(self):
        ...
        assert publisher.get_formatted_metrics([Timer("hello")]) == [b"hello:0|h"]
    ...
    def test_with_no_tags(self):
        ...
        assert publisher.get_formatted_metrics([counter, gauge, timer, histogram]) == [
            b"test.foo.timer.1:1|c",
            b"test.bar.gauge.1:5|g",
            b"test.baz.timer.1:2000|h",
            b"test.qux.histogram.1:13|h",
        ]
    ...
    def test_with_global_and_instrument_tags(self):
        ...
        assert (
            b"test.bar.gauge.1:5|g|#environment:qa,acceptance:None,jenkins-build:8293847,worker:52,extra:data,nothing:None,mail:snail,guitar:electric"
            in metrics
        )
        assert (
            b"test.baz.timer.1:2000|h|#environment:qa,acceptance:None,jenkins-build:8293847,number:5791.15,other_number:0"
            in metrics
        )
    ...
    def test_with_global_and_instrument_tags_and_distributions(self):
        ...
        assert (
            b"test.baz.timer.1:2000|d|#environment:qa,acceptance:None,jenkins-build:8293847,number:5791.15,other_number:0"
            in metrics
        )
        assert (
            b"test.qux.histogram.1:91|d|#environment:qa,acceptance:None,jenkins-build:8293847,extra:data,nothing:None,mail:snail,guitar:electric"
            in metrics
        )
